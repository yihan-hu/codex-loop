...
For an explicit GitHub -> Web request, the source path is fixed: exact target revision -> audited `.github/workflows/workspace-download.yml` or an exact receipt-bound published-source artifact -> commit-bound **Git bundle** -> GitHub Connector download -> artifact ZIP digest verification -> bundle SHA-256/size verification from the producing run -> `git bundle verify` -> materialization into a fresh real Git repository -> exact restored HEAD commit/tree verification. Do not replace this with shell `git clone`, per-file GitHub reconstruction, generic source archives, source-only `git archive`, or model-carried source merely because the container cannot reach GitHub directly. If a specialized workflow-run query cannot observe the relevant trigger class, continue through a compatible read-only repository Actions-runs endpoint and inspect exact import-run receipts/published-source artifacts within the same GitHub authority. Report `WORKSPACE_DOWNLOAD_ARTIFACT_UNAVAILABLE` only after that direct same-authority discovery is exhausted.

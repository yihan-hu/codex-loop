...
2. Generate the bridge source outside the Codex Loop Skill tree with `python3 scripts/build_self_update_bridge.py --output-dir OUT`. The generator must create exactly one separate Skill with a **fresh unique identity** of the form `codex-loop-update-bridge-<suffix>` for that recovery attempt; never use the unsuffixed `codex-loop-update-bridge` identity and never reuse a bridge name already present in the host catalog/library. The generator creates a random suffix by default; `--instance-id` exists only for deterministic tests or an explicitly chosen fresh suffix. Never embed the bridge `SKILL.md` inside the Codex Loop package.
3. Preserve the **proven-minimal install-compatible profile** exactly unless a new host experiment proves a replacement: the bridge package contains only `SKILL.md` and `agents/openai.yaml`; `allow_implicit_invocation` is `false`; `policy.products` is **absent**; and `interface.default_prompt` explicitly self-references the exact generated bridge name, for example `$codex-loop-update-bridge-a91d4`. The metadata absence and fresh identity are both evidence-backed compatibility state. On 2026-09-02, a new-name probe carrying `policy.products` reached Save but failed with `Library not found`; removing that field allowed the probe to save and persist. A later attempt using the corrected metadata but the already-present fixed identity `codex-loop-update-bridge` rendered only `Try in chat`, while the freshly named corrected probe remained installed and callable. Keep the Skill body minimal and functional: it may only require the already-validated canonical package, present that exact package through the host-native update surface, forbid Codex Loop invocation/package mutation, and terminate immediately after surface initiation. Do not add scripts, references, assets, connectors, or generalized updater behavior. Package it with Skill Creator's official `package_skill.py`.
4. Install the freshly named bridge through the same new-Skill Save surface proven by the corrected probe. If the host shows `Try in chat` instead of Save, treat that as an identity-collision signal: do not retry the same bridge name and do not mutate the canonical production package; generate a new fresh bridge identity with the same two-file/no-`policy.products` profile. A `Library not found` result from a probe or bridge that includes `policy.products` is a contaminated compatibility test, not evidence that the canonical Codex Loop ZIP or the account Library backend is invalid. Only if a fresh-name bridge with the proven metadata profile also fails may the broader host install/library surface be classified as unstable.
5. On a **later user/host turn**, explicitly invoke the installed bridge so that the host turn is owned by a different Skill identity. Supply the already-validated canonical Codex Loop `skill.zip` from the current conversation. The bridge itself presents that exact package through the native update surface; do not invoke Codex Loop or any Codex Loop runtime command in that bridge-owned update turn. Once the canonical update surface is initiated, end the turn immediately with no trailing status prose or second artifact.

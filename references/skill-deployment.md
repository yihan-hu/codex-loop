...
3. Preserve the **proven-minimal install-compatible profile** exactly unless a new host experiment proves a replacement: the bridge package contains only `SKILL.md` and `agents/openai.yaml`; `allow_implicit_invocation` is `false`; `policy.products` is **absent**; and `interface.default_prompt` explicitly self-references `$codex-loop-update-bridge`. This absence is evidence-backed compatibility state: the historical install-compatible Codex Loop metadata removed `policy.products`, and a 2026-09-02 host A/B reproduced `Library not found` when a disposable probe reintroduced it, then saved successfully after it was removed. Keep the Skill body minimal and functional: it may only require the already-validated canonical package, present that exact package through the host-native update surface, forbid Codex Loop invocation/package mutation, and terminate immediately after surface initiation. Do not add scripts, references, assets, connectors, or generalized updater behavior. Package it with Skill Creator's official `package_skill.py`.
4. Install the bridge under its different name. This step intentionally uses the host path already proven stable by the new-name probe. The disposable new-name probe used to establish that path must itself use the same install-compatible metadata rule: no `policy.products`. A `Library not found` result from a probe or richer bridge that includes `policy.products` is a contaminated compatibility test, not evidence that the canonical Codex Loop ZIP or the account Library backend is invalid. First reduce the test Skill to this proven profile; only if that profile also fails may the broader host install/library surface be classified as unstable.

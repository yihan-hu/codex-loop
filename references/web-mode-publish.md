...
Every successful FAST_PUBLISH must also close the **published-revision acquisition closure**: the same run uploads one self-contained `published-source-<run_id>` bundle after proving a fresh empty repository restores the exact published commit/tree. This artifact is for a later Web conversation; it is not the transport used for the current push and does not preserve the prior path/worktree. Under `repository-continuity.md` it is the normal clean `WARM_RESTORE_PUBLISHED_SOURCE` object when the HOT workspace is gone and no explicit Workspace Cache needs to preserve unpublished work. Reusing it is WARM restore, not cold source acquisition.

...
For the actual update, compose with the platform `skill-creator` workflow (or an explicitly equivalent native host-managed Skill update primitive) using the validated source/package generation bound to the same pushed commit. Codex Loop must never emulate a Save/Update UI in prose. **For Codex Loop self-update, this is a terminal self-update ownership transfer:** invoke the native installer as the final current-turn action, then stop Codex Loop execution and do not append a Codex Loop closing/status message. On a later user/host turn, run `skill-deploy-resume --later-host-turn-observed --evidence ...` for the exact revision before any `skill-deploy-surface-record` or other Codex Loop reconciliation. This is specifically intended to keep the Web native install control from being replaced by same-turn orchestrator output.

After that later-turn resume, when the host actually surfaced/initiated the native update control, record it with `skill-deploy-surface-record`; this may establish `UI_SURFACED` but still leaves `DEPLOY_PENDING`. Only after the intended revision is observably active may `skill-deploy-complete` establish `DEPLOYED`.

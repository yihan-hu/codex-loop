...
RDC is a host execution/interaction transport, never an authority bypass. Repository, browser/GUI, and ordinary configuration intents must be classified and routed through Codex Loop before their first RDC action. The one pre-admission exception is an explicitly Local objective's own lifecycle bootstrap/resume: RDC may touch only Codex Loop-owned local runtime paths (`~/.codex-loop/runtime-src` and `~/.codex-loop/runtime`) to establish the lifecycle before any repository inspection. This exception cannot read or mutate the target repository and cannot select Local mode by itself; it only realizes the user's already-explicit Local choice. Apply the repository boundary below when `workspace_mode=local`; apply the interaction-only boundary for `local_chrome`/`local_mac_gui`; use `route-check --action rdc_host_config` before the two narrow Codex Loop host config reads. See `interaction-routing.md`.
...
4. Task-owned scratch, artifact, release-staging, receipt, and temporary paths may be used only when they are explicitly derived inside the bound worktree or another already-authorized task path; do not infer sibling scratch roots from convenience. The narrow Codex Loop-owned lifecycle paths `~/.codex-loop/runtime-src` and `~/.codex-loop/runtime` are a separate infrastructure exception and may be touched only for lifecycle execution/state, never as general filesystem scope.
5. Treat every other location outside the Effective Local Roots as out of scope by default, including unrelated home-directory content, cloud-synced folders, Downloads, Desktop, Documents, credential stores, SSH configuration, package-manager caches, and system directories.

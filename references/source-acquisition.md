...
Treat direct-artifact discovery as a same-authority evidence problem, not as a single-wrapper lookup. If a specialized GitHub connector action only exposes a trigger class that cannot observe the required push or `workflow_dispatch` run, record `WORKSPACE_DOWNLOAD_OBSERVABILITY_UNAVAILABLE` for that query surface and continue through a compatible **same-authority read-only** GitHub observation. In particular, inspect repository Actions runs through a read-only endpoint that can represent the relevant trigger class, and inspect successful `Workspace Import` / `Workspace Import Fast` runs for a structured receipt whose `published_commit` and `published_tree` equal the exact target and whose bound `published-source-<run_id>` artifact has raw SHA-256/size plus `fresh_restore=PASS`. Do not invent a new transport, mutate source, or reconstruct files; this step only locates an already-produced direct Git bundle within the same GitHub authority.

Call `source-acquisition-plan` while discovery is still incomplete. With no direct artifact yet proven it returns `CONTINUE_DISCOVERY`, not a terminal blocker. Pass `--same-authority-artifact-discovery-exhausted` only after the compatible repository Actions-run lookup and the receipt-bound published-source lookup have both been actually checked for the exact target revision and neither yielded a usable direct artifact. Only then may the plan return `WORKSPACE_DOWNLOAD_ARTIFACT_UNAVAILABLE`. Do not automatically start incremental replay, per-file reconstruction, installed-Skill bootstrap, model relay, or Local mode merely because the direct artifact is missing. A fallback becomes eligible only after the host/model has observed an explicit **current-task user authorization** for that named fallback and passes `--current-user-fallback-authorization-observed` plus audit evidence. The authorization is current-task-only and must never be persisted as a preference.
...
An empty result from a connector action that only supports a different trigger class is `...OBSERVABILITY_UNAVAILABLE`, not proof that the workflow never ran. While a compatible same-authority read-only observation remains available, this is a continue-discovery condition rather than a terminal acquisition blocker.

...
class CliSmokeTests(unittest.TestCase):
  def test_end_to_end_host_validation_gate(self):
    with tempfile.TemporaryDirectory() as tmp:
      ...
      plan,_=call(root,'validate','--','pytest','-q'); gen=str(plan['data']['generation']); pid=plan['data']['plan_id']; call(root,'validation-record','--plan-id',pid,'--command-json','["pytest","-q"]','--generation',gen,'--exit-code','0','--evidence','host-visible pytest exited 0')
  ...
  def test_host_validation_plan_cannot_be_forged_or_reused(self):
    with tempfile.TemporaryDirectory() as tmp:
      ...
      plan,_=call(root,'validate','--','pytest','-q'); gen=str(plan['data']['generation']); pid=plan['data']['plan_id']

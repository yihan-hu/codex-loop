...
'''
    ...
    def test_generator_has_no_installer_maintenance_target_mode(self):
        with tempfile.TemporaryDirectory() as td:
            proc = subprocess.run(
                [
                    sys.executable,
            ...
            self.assertIn("unrecognized arguments", proc.stderr + proc.stdout)
    ...
    def test_docs_keep_bridge_as_explicit_recovery_fallback_only(self):
        ...
        self.assertIn("Installer companion maintenance ownership", deployment)
        self.assertIn("codex-loop -> codex-loop-install", deployment)
        self.assertIn("bridge_recovery_allowed=false", deployment)
        self.assertNotIn("--target-skill codex-loop-install", deployment)
        ...
        self.assertIn("Installing or updating `codex-loop-install`", readme)
        self.assertIn("owned by `codex-loop`", readme)
        self.assertNotIn("codex-loop-install-update-bridge-", readme)

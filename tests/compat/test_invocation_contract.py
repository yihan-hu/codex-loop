...
class InvocationContractTests(unittest.TestCase):
    ...
    def test_target_pursuit_is_opt_in_fail_closed_and_reuses_existing_completion_semantics(self):
        ...
        self.assertIn("Treat an unmodeled surprise that prevents the intended direct publish, deploy, install, or other external-action path as fail-closed before the external action", skill)
        self.assertIn("surface those choices and require the user's explicit selection", skill)
        self.assertIn("Make design repair the immediate next objective only when no modeled recovery option can satisfy the user's objective", skill)
        self.assertIn("Choosing a recovery path does not erase the original fast/direct-path defect", skill)

...
class ExecutionObservationTests(unittest.TestCase):
    def test_host_execution_policy_carries_rdc_safety_invariants(self):
        ...
        self.assertEqual(safety["default_max_task_owned_generated_file_bytes"], 1_000_000_000)
        self.assertTrue(safety["monitor_task_owned_generated_file_growth"])
        self.assertTrue(safety["generated_output_paths_must_be_identified_before_unbounded_write"])
        self.assertTrue(safety["file_producing_process_must_remain_observable_and_terminable"])
        self.assertTrue(safety["stop_writer_on_generated_file_limit"])
        self.assertTrue(safety["oversized_generated_file_requires_explicit_task_need_and_finite_cap"])
        self.assertFalse(safety["free_disk_space_admission_required"])

...
class WriteTests(unittest.TestCase):
  def setup(self,root,profile='regular'):
    store=create_store(root); store.configure_task(store.path.parent.name,'change',[],profile=profile, request_anchor='change'); capture_baseline(root,store); return store

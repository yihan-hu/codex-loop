...
class SourceAcquisitionPolicyTests(unittest.TestCase):
    ...
    def test_explicit_github_to_web_acquisition_preserves_git_identity(self):
        ...
        self.assertIn("source-acquisition-verify", skill)
        self.assertIn("source-acquisition-verify", acquisition)
        self.assertIn("before durable bootstrap or source mutation", skill)
        self.assertIn("same `.git` was rewritten onto disconnected history", skill)

...
class AuditBoundaryTests(unittest.TestCase):
    def test_failed_bootstrap_does_not_replace_active_task(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            first, _ = call(root, 'bootstrap', '--task-id', 'good', '--objective', 'good', '--request-anchor', 'good', '--no-validation','--no-validation-reason','test fixture has no meaningful executable validation')
            self.assertEqual(first['data']['task_id'], 'good')
            out, proc = call(root, 'bootstrap', '--task-id', 'bad', '--objective', 'bad', '--request-anchor', 'bad', '--profile', 'no_such_profile', check=False)
    ...
    def test_repo_local_hook_file_is_not_authority(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            call(root, 'bootstrap', '--objective', 'write', '--request-anchor', 'write', '--no-validation','--no-validation-reason','test fixture has no meaningful executable validation')
    ...
    def test_protected_preexisting_file_needs_explicit_override(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            store = create_store(root); store.configure_task(store.path.parent.name, 'change', [], request_anchor='change', requires_validation=False,no_validation_reason='test fixture has no meaningful executable validation'); capture_baseline(root, store)
    ...
    def test_non_git_mode_only_change_invalidates_generation(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            store = create_store(root); store.configure_task(store.path.parent.name, 'x', [], request_anchor='x', requires_validation=False,no_validation_reason='test fixture has no meaningful executable validation'); capture_baseline(root, store)
    ...
    def test_git_ignored_small_file_is_protected_and_invalidates_generation(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            store=create_store(root); store.configure_task(store.path.parent.name,'ignored freshness',[], request_anchor='ignored freshness',requires_validation=False,no_validation_reason='fixture')
    ...
    def test_git_ignored_python_bytecode_cache_does_not_invalidate_generation(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            store=create_store(root); store.configure_task(store.path.parent.name,'ignored cache freshness',[], request_anchor='ignored cache freshness',requires_validation=False,no_validation_reason='fixture')
            capture_baseline(root,store)
    ...
    def test_persisted_secret_like_text_is_redacted(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            store.configure_task(store.path.parent.name, 'fix token=abcDEF123456789 secret=topsecret123', [], request_anchor='fix token=abcDEF123456789 secret=topsecret123', requires_validation=False,no_validation_reason='test fixture has no meaningful executable validation')
    ...
    def test_external_state_machine_rejects_terminal_regression(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            store = create_store(root); store.configure_task(store.path.parent.name, 'x', [], request_anchor='x', requires_validation=False,no_validation_reason='test fixture has no meaningful executable validation')
    ...
    def test_external_terminal_failure_blocks_until_evidence_resolution(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            store = create_store(root); store.configure_task(store.path.parent.name, 'x', [], request_anchor='x', requires_validation=False,no_validation_reason='test fixture has no meaningful executable validation'); capture_baseline(root, store); store.set_criterion(0, 'pass', 'objective checked')
            aid = store.record_external('github_comment', 'planned', 'issue:1', action_class='external_non_idempotent')
            store.record_external('github_comment', 'dispatched', 'issue:1', action_class='external_non_idempotent', action_id=aid)
    ...
    def test_external_terminal_states_require_details(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp); store = create_store(root); store.configure_task(store.path.parent.name, 'x', [], request_anchor='x', requires_validation=False,no_validation_reason='test fixture has no meaningful executable validation')
    ...
    def test_unrelated_failure_needs_evidence_and_does_not_cancel_pass(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            store = create_store(root); store.configure_task(store.path.parent.name, 'x', [], request_anchor='x', requires_validation=True); capture_baseline(root, store)
            fail = host_validation(store, ['lint'], 1, cwd=root, evidence='host lint exit 1; failure is outside the requested behavior')
            store.resolve_validation(fail, 'unrelated_to_request', 'reviewed the failure and confirmed it concerns an untouched unrelated module')
    ...
    def test_unknown_task_service_lookup_does_not_create_task_dir(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp); subprocess.run(['git', 'init', '-q'], cwd=root, check=True)
            call(root, 'bootstrap', '--objective', 'x', '--request-anchor', 'x', '--no-validation','--no-validation-reason','test fixture has no meaningful executable validation')
    ...
    def test_cancel_then_cleanup_is_explicit_and_does_not_revert_workspace(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp); subprocess.run(['git', 'init', '-q'], cwd=root, check=True)
            b, _ = call(root, 'bootstrap', '--objective', 'x', '--request-anchor', 'x', '--no-validation','--no-validation-reason','test fixture has no meaningful executable validation'); tid = b['data']['task_id']
...
class HiddenReadBoundaryTests(unittest.TestCase):
    def test_hash_refuses_workspace_external_path(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as out:
            ...
            subprocess.run(['git','init','-q'],cwd=root,check=True); call(root,'bootstrap','--objective','hash guard', '--request-anchor', 'hash guard','--no-validation','--no-validation-reason','test fixture has no meaningful executable validation')
    ...
    def test_write_content_file_refuses_arbitrary_external_file(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as out:
            root=Path(tmp); outside=Path(out)/'secret.txt'; outside.write_text('secret')
            subprocess.run(['git','init','-q'],cwd=root,check=True); call(root,'bootstrap','--objective','write guard', '--request-anchor', 'write guard','--no-validation','--no-validation-reason','test fixture has no meaningful executable validation')
    ...
    def test_write_content_file_refuses_task_private_state(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); subprocess.run(['git','init','-q'],cwd=root,check=True)
            boot,_=call(root,'bootstrap','--objective','private read guard', '--request-anchor', 'private read guard','--no-validation','--no-validation-reason','test fixture has no meaningful executable validation')
...
class ValidationDispositionBoundaryTests(unittest.TestCase):
    def test_post_edit_failure_can_be_classified_unrelated_after_semantic_review(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); store=create_store(root); store.configure_task(store.path.parent.name,'x',[], request_anchor='x',requires_validation=True); capture_baseline(root,store)
            ...
            store.resolve_validation(vid,'unrelated_to_request','failure is in an untouched database integration and is not caused by the requested x.py change')
            self.assertEqual(store.validation_state_for_generation(store.generation())['nonblocking_count'],1)

    def test_unrelated_classification_requires_evidence_and_cannot_hide_a_pass(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); store=create_store(root); store.configure_task(store.path.parent.name,'x',[], request_anchor='x',requires_validation=True); capture_baseline(root,store)
            failed=host_validation(store,['broad-test'],1,cwd=root,evidence='broad test failed')
            with self.assertRaisesRegex(ValueError,'observable evidence'):
                store.resolve_validation(failed,'unrelated_to_request','')
            passed=host_validation(store,['targeted-test'],0,cwd=root,evidence='targeted test passed')
            with self.assertRaisesRegex(ValueError,'passing validation'):
                store.resolve_validation(passed,'unrelated_to_request','not applicable')

    def test_unrelated_classification_stales_when_effective_request_changes(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); store=create_store(root); store.configure_task(store.path.parent.name,'fix parser',[], request_anchor='fix parser only',requires_validation=True); capture_baseline(root,store)
            failed=host_validation(store,['pytest','tests/db'],1,cwd=root,evidence='database test failed')
            store.resolve_validation(failed,'unrelated_to_request','database behavior is outside the parser-only request')
            before=store.validation_state_for_generation(store.generation())
            self.assertEqual(before['failed_count'],0); self.assertEqual(before['nonblocking_count'],1)
            store.record_steer('also fix the database failure')
            after=store.validation_state_for_generation(store.generation())
            self.assertEqual(after['nonblocking_count'],0); self.assertEqual(after['stale_nonblocking_count'],1); self.assertEqual(after['failed_count'],1)
            self.assertIn('STALE_UNRELATED_VALIDATION_DISPOSITION',after['warning_codes'])
...
class CancelledTaskBoundaryTests(unittest.TestCase):
    ...
    def test_cancelled_write_rejects_before_payload_file_access(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); subprocess.run(['git','init','-q'],cwd=root,check=True)
            call(root,'bootstrap','--objective','cancel write boundary', '--request-anchor', 'cancel write boundary','--no-validation','--no-validation-reason','test fixture has no meaningful executable validation')
    ...
    def test_cancelled_task_rejects_progress_state_mutations(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); subprocess.run(['git','init','-q'],cwd=root,check=True)
            call(root,'bootstrap','--objective','cancel state boundary', '--request-anchor', 'cancel state boundary','--no-validation','--no-validation-reason','test fixture has no meaningful executable validation')
            call(root,'cancel','--reason','stop')
            for args in [
    ...
    def test_cancelled_task_cannot_plan_validation_or_start_new_service(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); subprocess.run(['git','init','-q'],cwd=root,check=True)
            call(root,'bootstrap','--objective','cancel boundary', '--request-anchor', 'cancel boundary','--no-validation','--no-validation-reason','test fixture has no meaningful executable validation')
            call(root,'cancel','--reason','stop')
    ...
    def test_cancelled_task_rejects_review_checkpoint_snapshot_and_validation_resolution(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); subprocess.run(['git','init','-q'],cwd=root,check=True)
            call(root,'bootstrap','--objective','cancel progress boundary', '--request-anchor', 'cancel progress boundary','--no-validation','--no-validation-reason','test fixture has no meaningful executable validation')
            call(root,'cancel','--reason','stop')
            for args in [
    ...
    def test_cancelled_task_allows_only_already_dispatched_external_outcome_reconciliation(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); subprocess.run(['git','init','-q'],cwd=root,check=True)
            call(root,'bootstrap','--objective','cancel external boundary', '--request-anchor', 'cancel external boundary','--no-validation','--no-validation-reason','test fixture has no meaningful executable validation')
    ...
    def test_cancel_is_not_a_progress_mutation_after_cancellation(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); subprocess.run(['git','init','-q'],cwd=root,check=True)
            call(root,'bootstrap','--objective','cancel once', '--request-anchor', 'cancel once','--no-validation','--no-validation-reason','test fixture has no meaningful executable validation')
    ...
    def test_cancel_closes_planned_but_not_dispatched_external_actions(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); subprocess.run(['git','init','-q'],cwd=root,check=True)
            call(root,'bootstrap','--objective','cancel planned external', '--request-anchor', 'cancel planned external','--no-validation','--no-validation-reason','test fixture has no meaningful executable validation')
...
class ModifiedDesignBoundaryTests(unittest.TestCase):
    def test_task_scoped_cli_inherits_active_identity_and_fails_without_binding(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); subprocess.run(['git','init','-q'],cwd=root,check=True)
            boot,_=call(root,'bootstrap','--objective','identity', '--request-anchor', 'identity','--no-validation','--no-validation-reason','test fixture has no meaningful executable validation'); tid=boot['data']['task_id']
...
class BootstrapIdentityBoundaryTests(unittest.TestCase):
    def test_bootstrap_refuses_existing_explicit_task_id(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); subprocess.run(['git','init','-q'],cwd=root,check=True)
            first=subprocess.run([sys.executable,str(CLI),'bootstrap','--cwd',str(root),'--task-id','fixed_task','--objective','first', '--request-anchor', 'first','--no-validation','--no-validation-reason','test fixture has no meaningful executable validation'],stdout=subprocess.PIPE,stderr=subprocess.PIPE,check=True)
            self.assertTrue(json.loads(first.stdout)['ok'])
            second=subprocess.run([sys.executable,str(CLI),'bootstrap','--cwd',str(root),'--task-id','fixed_task','--objective','second', '--request-anchor', 'second','--no-validation','--no-validation-reason','test fixture has no meaningful executable validation'],stdout=subprocess.PIPE,stderr=subprocess.PIPE)

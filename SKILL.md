...
After every successful GitHub source push whose remote readback matches the audited commit/tree, generate a ChatGPT-chatbox handoff with `python3 scripts/codex_loop.py chatbox-handoff --repository OWNER/REPO --commit FULL_SHA`. Append the returned `copy_prompt` verbatim at the end of the user-facing push report and tell the user to copy/send it as their next chatbox message. This is a user-message URL handoff for later workspace download, not proof of deployment and not permission to switch to GitHub connector, browser UI, local Mac artifacts, cloud-storage relays, or model-carried bytes. Read the exact contract in `references/skill-deployment.md`.

---
name: codex-loop
description: "Apply a Codex-style coding-agent loop for end-to-end repository work: implement features, fix bugs/tests/CI, refactor, investigate, review, package/release, publish, or run reviewer/researcher/tester workflows. New conversations start in the current ChatGPT/web workspace. Local repository development is selected only by explicit local/PiWork intent; local source mutation and local computer use each require explicit current-task authorization, and RDC/browser requests do not switch workspace mode. Supports local Chrome/macOS interaction routing, task-start capability preflight, persisted host-local LOCAL_ROOT defaults, validation/review/completion gating, verified Web-mode GitHub publishing through Google Drive staging + audited GitHub Actions, verified Local-mode native-Git publishing, optional post-push workspace sync, and Skill packaging/deployment separation. Do not use merely to explain code when no repository action is needed; never launch Codex CLI, Codex App Server, or another model runtime."
---
...
When Local mode is selected, resolve `LOCAL_ROOT` as the user's absolute RDC-authorized persistent development root. Reuse the resolved root for later repository tasks in the same conversation. If the user chose to persist a default root across conversations, consult `~/.codex-loop/host.json` only after explicit Local-development intent; the config never selects Local mode by itself. If the root remains unresolved or is not authorized by RDC, fail closed and ask once for the exact root. Read `references/local-mode-setup.md` before the first repository-affecting local filesystem operation in a conversation.
...
Treat source publication, Skill packaging, and ChatGPT deployment as separate stages. Read `references/skill-deployment.md` whenever a task installs/updates any Skill or Skill installation package, moves artifacts between ChatGPT and the local host, or raises a synchronization question. In Web mode, keep the current ChatGPT workspace as the task source baseline; return downloadable artifacts normally, and when GitHub publication is requested use the verified `references/web-mode-publish.md` path without migrating the task to Local mode. In local mode, keep the canonical Git repository under `LOCAL_ROOT` as source of truth and GitHub as the durable remote. Treat `skill.zip` as a release artifact and the installed ChatGPT Skill as a deployed copy in either mode. For **any** Skill or Skill installation package already present in the current workspace or host-managed Skill environment, do not interpret install/update/“UI install” wording as authorization to automate ChatGPT's browser UI. Reuse the existing workspace/host resource and perform the requested source/package update or publication through supported non-browser capabilities first. Actual ChatGPT Skills UI interaction requires an explicit request for that UI operation and, if computer use is needed, explicit computer-use authorization. Otherwise never assume Git push or local packaging automatically updates a separately installed Skill. If a required binary transfer bridge is unavailable, explain the exact boundary to the user and request real file placement/transfer or explicit authorization for a specific alternate data plane. Do not simulate missing file transfer by chunking source/archive bytes through model text, base64, heredocs, repeated remote writes, or connector payloads unless the user explicitly authorizes that exact method after the limitation is disclosed.

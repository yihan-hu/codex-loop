---
name: codex-loop
description: Apply a Codex-style coding-agent loop with a deterministic local runtime for repository work. Default to development in the current ChatGPT/web workspace and return downloadable artifacts there. Use the persistent PiWork workspace through Remote Desktop Commander and native Git only when the user explicitly requests local/PiWork development. Supports verified local GitHub publishing and optional post-push synchronization back into the current ChatGPT workspace, alongside the existing validation, review, release, delegation, and completion controls. Use for end-to-end repository work such as implementing features, fixing bugs, repairing tests or CI, refactoring, investigation, review, release/package preparation, publishing repository changes, or workflows that request a reviewer/researcher/tester subagent. Do not use merely to explain code when no repository action is needed, and never use it to launch or delegate to Codex CLI, Codex App Server, or another model runtime.
---
...
Default to **web mode**: develop in the current ChatGPT/web workspace using the files and tools already available there, and return generated files with normal workspace download links. Do not inspect, mutate, or synchronize `/Users/yihanhu/PiWork` merely because Remote Desktop Commander is available.

Enter **local mode** only when the user explicitly asks for local development, PiWork, Remote Desktop Commander, or an equivalent persistent-Mac workflow. A generic request such as `push` does not silently migrate a web-mode task into PiWork. If verified publishing would require local mode, keep the web result intact and state that local mode is required rather than moving the source without authorization.

Development location is task-scoped, not a permanent preference. Once local mode is explicitly selected for the current task, keep that task on its bound PiWork Git worktree through edits, validation, commit, and any requested native-Git publishing.

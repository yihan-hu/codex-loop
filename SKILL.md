...
**Web source acquisition gate.** When Web mode needs to materialize repository source from GitHub into the current ChatGPT workspace, read `references/source-acquisition.md` before source transfer. The standard GitHub -> Web path is exact GitHub revision -> audited `.github/workflows/workspace-download.yml` -> commit-bound GitHub Actions source artifact -> GitHub Connector artifact download -> artifact digest and source-archive SHA-256 verification -> extraction into a fresh Web workspace. Do not use shell `git clone`, per-file GitHub contents/blob reconstruction, generic repository archive URLs, or model-carried source as the ordinary Web acquisition path. If an exact commit-bound workflow run cannot be produced or observed, report that capability precisely instead of inferring that the workflow failed or silently switching transports.
...
Bind every durable task to the workspace implied by the current conversation's development mode. In web mode, the current ChatGPT workspace is the mutable development baseline. When local mode is active for the conversation, bind each task to the selected Git working tree under `LOCAL_ROOT` and treat that working tree as the only mutable local baseline. An installed Skill directory is normally deployment state, not development source, but it may be used as a **one-time bootstrap source** when current evidence proves that the installed copy is bound to the exact intended canonical repository revision and that revision is still the latest observed target-branch HEAD. Copy the verified installed Skill into a fresh workspace before the first mutation, record the repository/commit/freshness evidence, then treat only the new workspace as mutable authority; never edit the installed directory in place or continue consulting it as a competing source. Without exact freshness/provenance evidence, use the verified GitHub -> Web acquisition path instead. Copied `final`/`publish` folders, release staging directories, downloaded synchronization artifacts themselves, and unpacked release artifacts remain transport/release material rather than later development baselines. For concurrent local tasks on one repository, use separate Git branches/worktrees that share repository history rather than full-source copies.

...
**Installing or updating Codex Loop in ChatGPT Web.** Default to the fixed installed `codex-loop-install` companion. It validates the exact provenance-bound production package and drives the host-native update surface while the running `codex-loop` revision is quiescent. Install the companion once; do not create a new bridge Skill for every update. If the companion is missing or the host surface fails, stop with `DEPLOY_PENDING` and the exact blocker. The legacy fresh-name bridge is an explicit user-requested recovery fallback only, and using it necessarily creates a temporary visible Library entry.

...
For **Codex Loop updating itself**, `skill-deploy-handoff --routing-session-id ROUTING_SESSION` is now result-preserving planning: it records `INSTALL_READY` but does not activate the terminal barrier, so the publication/validation/result response remains fully visible. On a later install-only turn, `skill-deploy-install-begin` activates the terminal self-update barrier; only then must `skill-creator`/the native installer be the final owner and Codex Loop must not issue another command after the install surface is initiated. Reconciliation starts only on a later user/host turn. When that turn is in the same conversation, use `skill-deploy-resume --later-host-turn-observed --same-conversation-observed` and keep using the returned `routing_session_id`; this preserves still-fresh scoped capability observations instead of repeating permission smoke. A genuinely new conversation starts a new route. This prevents both output loss in the result-bearing turn and orchestrator takeover of the native install turn while preserving FAST_PUBLISH continuity.
...
**The same-name Skill update UI appears and immediately disappears.** Validate the production package first. If it passes, allow at most one exact-content repack with Skill Creator's official packager, then use a minimal disposable new-name Skill as an A/B host-surface probe. If the new-name install UI is stable while the same-name update UI disappears, classify `HOST_SAME_NAME_SKILL_UPDATE_SURFACE_UNSTABLE`, stop modifying the canonical package to chase the symptom, and keep the probe diagnostic-only. Do not rename the production Skill or remove/replace the installed Skill without a separate explicit deployment instruction. See `references/skill-deployment.md`. After that diagnosis succeeds, use a one-shot **Codex Loop Update Bridge** whose identity is fresh for every recovery attempt, for example `codex-loop-update-bridge-a91d4`. Preserve the experimentally successful install-compatible bridge shape: exactly two files (`SKILL.md`, `agents/openai.yaml`), explicit-only invocation, **no `policy.products`**, and a `default_prompt` that self-references that exact fresh bridge name. The unsuffixed `codex-loop-update-bridge` identity must not be reused when it is already present in the host catalog: a 2026-09-02 host A/B showed that corrected metadata plus the fixed identity rendered only `Try in chat`, while a freshly named corrected probe saved, persisted, and remained callable. A `Library not found` result from a probe or bridge carrying `policy.products` is a contaminated compatibility test and must be retried without that field before blaming the production ZIP or host Library backend. If a bridge shows `Try in chat` instead of Save, generate a new fresh bridge identity with the same minimal metadata profile rather than retrying that name. On a later bridge-owned turn present only the canonical Codex Loop package for update and terminate after the native surface starts.

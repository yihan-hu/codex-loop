...
**Codex Loop self-update UI disappears or Library Save says `Library not found`.** Use one path only: keep the validated production `codex-loop` package unchanged, generate a fresh bridge with `scripts/build_self_update_bridge.py`, preserve the user-verified `b5a748` two-file envelope, run the pinned regression plus Skill Creator validation, package it with the official packager, and Save that fresh bridge through the native Library surface. A successful Save ends the assistant-side recovery guidance: **do not emit a follow-up bridge command** or offer another recovery route. The saved bridge stays explicit-only and may present only the unchanged canonical production package if the host later invokes it. Do not document or offer a second recovery path alongside this one.

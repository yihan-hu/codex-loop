...
**Updating `codex-loop-install` shows `Library not found`.** This is the one fixed fresh-name recovery exception. Keep the validated installer package unchanged, generate a fresh explicit-only bridge with `python3 scripts/build_self_update_bridge.py --output-dir OUT --target-skill codex-loop-install`, validate/package it with Skill Creator, save that new bridge through the Library surface, then invoke it once so it presents the exact installer package through the host-native update surface. This repairs the permanent installer only; after it is healthy, normal Codex Loop updates go back to the bridge-free fixed-installer path. Do not retry the same failed installer package as a Library Save and do not loop if a fresh recovery bridge itself also gets `Library not found`.

...
For **Codex Loop installing/updating itself in ChatGPT Web**, use the verified production `codex-loop` package through the native same-name Skill update surface. `skill-deploy-handoff --routing-session-id ROUTING_SESSION` records `INSTALL_READY`, `BRIDGE_NOT_SELECTED`, `install_strategy=native_same_name_update`, and `native_self_update_attempt_allowed=true` without activating the terminal barrier. On the later install-only turn, `skill-deploy-install-begin` activates the barrier; the final action is invoking `skill-creator`/the host-native same-name update with the production package. Reconciliation starts only on a later user/host turn. When that turn is in the same conversation, use `skill-deploy-resume --later-host-turn-observed --same-conversation-observed` and keep using the returned `routing_session_id`.

Source publication, native update-surface observation, and production deployment are separate evidence states. Codex Loop must not emulate the product UI in prose. A bridge Skill is **not created automatically**; an uninstalled Skill cannot be treated as a hidden/temporary executable bridge unless the host provides a real ephemeral Skill primitive. The legacy bridge generator is explicit recovery only, and saving that fallback creates a visible Library item. Browser automation remains separately authorized.
...
**A new Skill version is not active after pushing.** For the active Skill being maintained in the current Web workspace, this should remain an unfinished `DEPLOY_PENDING` workflow until the intended revision is observably active. `skill-deploy-handoff` alone never counts as UI evidence. For Codex Loop self-update, use the native same-name update of the verified production package, then on the next user/host turn run `skill-deploy-resume` and reconcile surface/deployment evidence with `skill-deploy-surface-record` and `skill-deploy-complete`. Do not auto-create a bridge Skill.

**Installing or updating Codex Loop in ChatGPT Web.** Default to the verified production package through the native same-name update surface; never create a second Library Skill automatically. If the host cannot perform that update, stop with `DEPLOY_PENDING` and the exact host blocker. The legacy fresh-name bridge is an explicit user-requested recovery fallback only, and using it necessarily creates a temporary visible Library entry.

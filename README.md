...
For **Codex Loop installing/updating itself in ChatGPT Web**, do **not** try the standard same-name/native production Skill update. Every install goes directly through the user-verified fresh-name Library bridge strategy. `skill-deploy-handoff --routing-session-id ROUTING_SESSION` records `INSTALL_READY`, `BRIDGE_REQUIRED`, `install_strategy=verified_library_bridge`, and `native_self_update_attempt_allowed=false` without activating the terminal barrier. Before the install-only turn, generate a fresh bridge with `scripts/build_self_update_bridge.py` (no `--instance-id`), run the pinned bridge regression, validate it with Skill Creator, and package it with the official packager while keeping the canonical production `skill.zip` unchanged. On the later install-only turn, `skill-deploy-install-begin` activates the terminal barrier; the final action is saving that prepared fresh bridge through the host Library surface. Reconciliation starts only on a later user/host turn. When that turn is in the same conversation, use `skill-deploy-resume --later-host-turn-observed --same-conversation-observed` and keep using the returned `routing_session_id`. Change this self-install strategy only when the user explicitly asks to change it.

Source publication, bridge/Library surface observation, and production deployment are separate evidence states. Codex Loop must not emulate the product UI in prose: after the later-turn resume, `skill-deploy-surface-record` is valid only when the host actually exposed/saved the prepared fresh bridge, and `skill-deploy-complete` still requires evidence that the intended production Codex Loop revision is active. Browser automation remains separately authorized.
...
For repeated small Web pushes, Codex Loop treats `push`/`publish` as a **delivery continuation**, not a semantic steer, unless the user also changes source requirements. It first runs `web-publish-continuation-begin`; if the current clean generation already has fresh validation/review, the runtime marks them reusable and rejects redundant `validate` calls until content changes. It then uses `web-publish-plan` as the zero-waste gate, with FAST_PUBLISH as the command default; standard Web publication requires explicit `--standard-web`. If exact-scope capability observations are still fresh and the remote head is a locally provable ancestor, the plan selects one thin Git bundle directly from that remote head. The fast path performs no repeated permission smoke, no repeated validation/review, no failed full-bundle attempt, and no production Skill packaging during intermediate fix -> push -> inspect cycles. If only freshness is missing it returns `FAST_PUBLISH_REFRESH_REQUIRED` with the exact gates to refresh, and the standard importer remains forbidden unless the user explicitly selects it. Production packaging/deployment is deferred until the performance loop itself passes.
...
`web-publish-bundle` creates the exact bundle. A verified remote base may be supplied as a Git bundle prerequisite when an acquired Web workspace intentionally lacks older history; the prerequisite must be an exact ancestor of audited HEAD. `web-publish-plan` defaults to FAST_PUBLISH and may reuse fresh validation/review/capability evidence and an unchanged bundle receipt; `--standard-web` is the only CLI opt-in to the full standard Web importer. The remote short-circuit is valid only when **both** commit and tree already equal the audited source.
...
**A new Skill version is not active after pushing.** For the active Skill being maintained in the current Web workspace, this should remain an unfinished `DEPLOY_PENDING` workflow until the intended revision is observably active. `skill-deploy-handoff` alone never counts as UI evidence. For ordinary Skills, use the normal host-native update surface. For Codex Loop self-update, do not try that route: prepare/save the fresh Library bridge, then on the next user/host turn run `skill-deploy-resume` and reconcile bridge-surface/deployment evidence with `skill-deploy-surface-record` and `skill-deploy-complete`.

**Installing or updating Codex Loop in ChatGPT Web.** Always use one path from the start: keep the validated production `codex-loop` package unchanged, generate a fresh bridge with `scripts/build_self_update_bridge.py`, preserve the user-verified `b5a748` two-file envelope (including its `Library not found` wording), run the pinned regression plus Skill Creator validation, package it with the official packager, and Save that fresh bridge through the native Library surface. **Do not first try a standard same-name/native production update.** A successful Save ends the assistant-side bridge guidance: **do not emit a follow-up bridge command** or offer another install route. The saved bridge stays explicit-only and may present only the unchanged canonical production package if the host later invokes it.

...
**Codex Loop self-update UI disappears or bridge Save says `Library not found`.** Keep the validated production `codex-loop` package unchanged and use the single host-tested recovery path in `references/skill-deployment.md`: generate a fresh bridge with `scripts/build_self_update_bridge.py`, which is pinned to the user-verified `b5a748` Library-safe envelope, package it with Skill Creator's official packager, Save it, and invoke that exact bridge on a later turn to present only the canonical production package. The bridge has exactly `SKILL.md` and `agents/openai.yaml`, quoted UI metadata, explicit-only invocation, and no `policy.products`. Do not use alternate probe/bridge profiles unless a new host test explicitly supersedes this template.
